...
float kickImpulse = 0.0f;
float cameraKick = 0.0f;
float cameraKickVelocity = 0.0f;
float lowpassState = 0.0f;
float kickEnergyShort = 0.0f;
float kickEnergyLong = 0.0f;
           ...
           double streamTime, RtAudioStreamStatus status, void* noData )
{
  ...
  pthread_mutex_lock(&g_mutex);
	  ...
	  //Copy input buffer to callbackFilledAudioBuffers and output (pass through to speakers)
		...
		memset(oSamples, 0, nBufferFrames * 2 * sizeof(SAMPLE));	//Assumes 2 channels in / out
...
{	
	...
	glClearColor(0.13f, 0.13f, 0.14f, 1.0f); // grey color
	glEnable(GL_BLEND);
	...
	GLfloat fogColor[4] = { 0.14f, 0.16f, 0.18f, 1.0f };
 ...
 //Bufferframs = 2048 -> fade block alpha 0.15
	// RtAudio Variables ------------------------------------------------------------
  unsigned int inChannels = 2, outChannels = 2, fs = SRATE, bufferFrames = 1024, iDevice = 0, oDevice = 0, iOffset = 0, oOffset = 0;
  ...
  if (audio.openStream(&oParams, &iParams,
	  RTAUDIO_FLOAT64,
	  fs,
	  &bufferFrames,
	  &tick,
...
void displayFunc( )
{
	...
	SAMPLE* audioL = autocorrBuffer; // tu as déjà le buffer gauche

	float energy = 0.0f;
	int kickSamples = 128; // un peu plus large

	float cutoff = 0.15f; // PLUS PETIT = plus grave (clé du filtrage)

	for (int i = 0; i < kickSamples; ++i)
	{
		float s = autocorrBuffer[i];

		// filtre passe-bas 1-pôle
		lowpassState += cutoff * (s - lowpassState);

		energy += lowpassState * lowpassState;
	}

	energy /= kickSamples;
	kickEnergyShort = 0.6f * kickEnergyShort + 0.4f * energy; // rapide
	kickEnergyLong = 0.98f * kickEnergyLong + 0.02f * energy; // lent


	// lissage
	kickEnergy = 0.9f * kickEnergy + 0.1f * energy;

	// seuil empirique (à ajuster légèrement)
	if (kickEnergyShort > kickEnergyLong * 2.8f)
	{
		kickImpulse = 1.0f;
	}

	float kickStrength = 0.8f;   // puissance du coup
	float damping = 0.85f;       // retour doux

	if (kickImpulse > 0.0f)
	{
		cameraKickVelocity += kickStrength * kickImpulse;
		kickImpulse = 0.0f;
	}

	cameraKickVelocity *= damping;
	cameraKick += cameraKickVelocity;

	// retour à zéro
	cameraKick *= 0.9f;

	float drive = kickEnergyShort; // ou énergie basse
...
if( show3D )
{
 ...
 // -------------------- Draw the Coolness  -------------------------- 	
	...
	float baseZ = 14.0f;
	float kickZ = baseZ - cameraKick * 3.5f; // coup vers l’avant
    ...
    glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT ); 
	// Additive blending for 3D lines
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    glPushMatrix();
    	...
    	for (GLint i = 0; i < autocorrHandler->getN() - autocorrSkip1*5-4; i += 4)
    	{
    	...
    	for( GLint i = 0; i < autocorrHandler->getN() - autocorrSkip2*5-4; i=i+6 )
    	{
...
//	   			glColor4f(0.0 , 136/255.0, 158/255.0, pow(0.8, plotNum)*0.7); //2048 buffersize
	   			glColor4f(0.45f, 0.65f, 0.55f, pow(0.89, plotNum)*0.7); //1024 buffersize
...
//	   			glColor4f(1.0, 1.0, 1.0, pow(0.8, plotNum)*0.8);	//2048 buffersize
	   			glColor4f(0.85f, 0.85f, 0.85f, pow(0.94f, plotNum)*0.45f);
